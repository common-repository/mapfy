...
if ( ! class_exists( 'Subscribe' ) ) {
	...
	class Subscribe extends Notice {
		...
		public function __construct() {
			parent::__construct();
			add_action( 'wp_ajax_mapfy_subscribe', array( $this, 'mapfy_subscribe' ) );
		}
		 ...
		 * @author Jewel Theme <support@jeweltheme.com>
		 */
		public function mapfy_subscribe() {
			check_ajax_referer( 'mapfy_subscribe_nonce' );
		...
		public function notice_content() {
			...
			?>
			<div class="notice notice-jltmapfy is-dismissible notice-plugin-review notice-<?php echo esc_attr( $this->color ); ?> jltmapfy-notice-<?php echo esc_attr( $this->get_id() ); ?>">
				<button type="button" class="notice-dismiss jltmapfy-notice-dismiss"></button>
				<img width="70" height="70" src="<?php echo esc_url( MAPFY_IMAGES . '/logo.png' ); ?>" alt="<?php esc_attr_e( 'Logo', 'mapfy' ); ?>">
				<div class="jltmapfy-subscribe-content">
					<?php $this->set_title(); ?>
					<form style="display:flex">
						<div class="jltmapfy-plugin-subscribe-input">
						...
						<button type="submit" class="button button-primary jltmapfy-subscribe-btn"><?php esc_html_e( 'Get Discount', 'mapfy' ); ?></button>
					</form>
				</div>
			</div>
			<?php
		...
		public function plugin_rate_url() {
			return 'https://wordpress.org/plugins/' . MAPFY_SLUG;
		}
		...
		public function footer_content() {
			...
			<a class="button button-primary rate-plugin-button" href="<?php echo esc_url( $this->plugin_rate_url() ); ?>" rel="nofollow" target="_blank">
				<?php echo esc_html__( 'Rate Now', 'mapfy' ); ?>
			</a>
			<a class="button notice-review-btn review-later jltmapfy-notice-dismiss" href="#" rel="nofollow">
				<?php echo esc_html__( 'Later', 'mapfy' ); ?>
			</a>
			<a class="button notice-review-btn review-done jltmapfy-notice-disable" href="#" rel="nofollow">
				<?php echo esc_html__( 'I already did', 'mapfy' ); ?>
			</a>
			<?php
		}
		...
		public function core_script( $trigger_time ) {
			...
			<script>
				jQuery(document).on('submit', '.jltmapfy-notice-subscribe .jltmapfy-subscribe-content form', function(e) {
					...
					let formWrapper = form.closest('.jltmapfy-subscribe-content');
					...
					jQuery.ajax({
							...
							crossDomain: true,
							data: {
								action: 'mapfy_subscribe',
								_wpnonce: '<?php echo esc_js( wp_create_nonce( 'mapfy_subscribe_nonce' ) ); ?>',
								name: name,
								email: email,
							}
						})
						.done(function(response) {
							formWrapper.hide().after('<p class="jltmapfy--notice-message"><strong>' + response.data + '</strong><p>');
							let subsTimeout = setTimeout(function() {
								mapfy_notice_action(null, form, 'disable');
