...
class Notifications {
	...
	public function __construct() {
		...
		add_action( 'mapfy_display_notice', array( $this, 'display_notice' ), 10, 2 );
		add_action( 'mapfy_display_popup', array( $this, 'display_popup' ), 10, 2 );

		add_action( 'wp_ajax_mapfy_notification_action', array( $this, 'notification_action' ) );
	}
	...
	public function notification_action() {
		check_ajax_referer( 'mapfy_notification_nonce' );
	...
	public function setup_notifications_by_type( $type ) {
		...
		// Block if necessary .
		$notification_last_fired = get_option( "mapfy_{$type}_last_interact" );
		...
		do_action( "mapfy_display_{$type}", $notification, $trigger_time );
	}
	...
	public function display_popup( $popup, $trigger_time ) {
		...
		<div class="jltmapfy-popup" id="jltmapfy-popup" data-plugin="<?php echo esc_attr( $this->slug ); ?>" tabindex="1">

			<div class="jltmapfy-popup-overlay"></div>

			<div class="jltmapfy-popup-modal" style="background-image: url('<?php echo esc_url( $image_url ); ?>'); --jltmapfy-popup-color: <?php echo esc_attr( $popup->get_content( 'btn_color' ) ); ?>;">
				...
				<div class="jltmapfy-popup-modal-footer">
					...
					<div class="jltmapfy-popup-countdown" style="display: none;">
						<span class="jltmapfy-popup-countdown-text"><?php echo esc_html__( 'Deal Ends In', 'mapfy' ); ?></span>
					...
					<!-- button  -->
					<a class="jltmapfy-popup-button" target="_blank" href="<?php echo esc_url( $popup->get_content( 'button_url' ) ); ?>"><?php echo esc_html( $popup->get_content( 'button_text' ) ); ?></a>
				</div>
			</div>
		</div>

		<script>
			function mapfy_popup_action(evt, $this, $action_type) {

				evt.preventDefault();

				$this.closest('.jltmapfy-popup').fadeOut(200);
			...
			// Notice Dismiss
			jQuery('body').on('click', '.jltmapfy-popup .popup-dismiss', function(evt) {
				mapfy_popup_action(evt, jQuery(this), 'dismiss');
			});

			// Notice Disable
			jQuery('body').on('click', '.jltmapfy-popup .popup-disable', function(evt) {
				mapfy_popup_action(evt, jQuery(this), 'disable');
			});
		</script>
