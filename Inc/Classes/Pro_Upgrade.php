...
use MAPFY\Inc\Classes\Notifications\Base\Date;
...
class Pro_Upgrade
{
	...
	{
		$this->slug = Helper::mapfy_slug_cleanup();
	...
	public function dashboard_widget()
	{
		wp_add_dashboard_widget(
			'mapfy_dashboard_widget',                          // Widget slug.
			esc_html__('Mapfy News & Updates', 'mapfy'), // Title.
		...
		$example_widget_backup = array('mapfy_dashboard_widget' => $default_dashboard['mapfy_dashboard_widget']);
		unset($default_dashboard['mapfy_dashboard_widget']);
	...
	public function dashboard_widget_render()
	{
		...
		echo '<div class="jltmapfy-rss-widget">';

		if (wp_validate_boolean($this->get_content('is_campaign'))) { ?>

			<div class="jltmapfy-dashboard-promo" style="--jltmapfy-popup-color: <?php echo esc_attr($this->get_content('btn_color')); ?>;">
				<a target="_blank" href="<?php echo esc_url($this->get_content('button_url')); ?>">
					<img src="<?php echo esc_url($this->get_content('image_url')); ?>" alt="Mapfy Promo Image" style="width: 100%; height: auto;">
				</a>

				<a class="jltmapfy-popup-button" target="_blank" href="<?php echo esc_url($this->get_content('button_url')); ?>">
		...
		if (!empty($maxitems)) {
			...
			foreach ($rss_items as $item) {
				...
				// $item_date = human_time_diff( $item->get_date( 'U' ), current_time( 'timestamp' ) ) . ' ' . __( 'ago', 'mapfy' );
		...
		<div class="jltmapfy-dashboard_footer">
			<ul>
				<li class="jltmapfy-overview__blog">
					<a href="https://jeweltheme.com/blog" target="_blank">
						Blog
						<span class="screen-reader-text">
							<?php echo esc_html__('(opens in a new window)', 'mapfy'); ?>
						</span>
						<span aria-hidden="true" class="dashicons dashicons-external"></span>
					</a>
				</li>
				<li class="jltmapfy-overview__help">
				...
				<li class="jltmapfy-overview__upgrade">
		...
		<style>
			...
			.jltmapfy-rss-widget .hndle.ui-sortable-handle img {
				margin: -5px 10px -5px 0;
			}

			.jltmapfy-rss-widget .jltmapfy-dashboard_footer {
				margin: 0 -12px -12px;
				padding: 0 12px;
				border-top: 1px solid #eee;
			}

			.jltmapfy-rss-widget .jltmapfy-dashboard_footer ul {
				display: flex;
				list-style: none;
			}

			.jltmapfy-rss-widget .jltmapfy-dashboard_footer ul li:first-child {
				padding-left: 0;
				border: none;
			}

			.jltmapfy-rss-widget .jltmapfy-dashboard_footer li {
				padding: 0 10px;
				margin: 0;
				border-left: 1px solid #ddd;
			}

			.jltmapfy-rss-widget .jltmapfy-overview__go-pro a {
				color: #FCB92C;
				font-weight: 500;
			}
		</style>
	...
	{
		return get_option('mapfy_sheet_promo_data');
	}
	...
	{
		return get_option('mapfy_sheet_promo_data_hash');
	}
	...
	{
		$hook_action = 'mapfy_sheet_promo_data_remote_sync';
		...
		register_deactivation_hook(MAPFY_FILE, array($this, 'clear_register_sync_hook'));
	}
	...
	{
		wp_clear_scheduled_hook('mapfy_sheet_promo_data_remote_sync');
	}
	...
	public function sheet_data_remote_sync()
	{
		...
		if ($force || $sheet_hash_data !== $sheet_data_hash) {
			update_option('mapfy_sheet_promo_data', $remote_data);
			update_option('mapfy_sheet_promo_data_hash', $sheet_data_hash);
			do_action('mapfy_sheet_promo_data_reset');
		}
	}
	...
	public function get_sheet_promo_remote_data()
	{
		...
		$plugin_slug = Helper::mapfy_slug_cleanup();
	...
	?>
		...
		<div class="jltmapfy-popup jltmapfy-upgrade-popup" id="jltmapfy-popup" data-plugin="<?php echo esc_attr($this->slug); ?>" tabindex="1" style="display: none;">

			<div class="jltmapfy-popup-overlay"></div>

			<div class="jltmapfy-popup-modal" style="background-image: url('<?php echo esc_url($image_url); ?>'); --jltmapfy-popup-color: <?php echo esc_attr($this->get_content('btn_color')); ?>;">

				<!-- close  -->
				<div class="jltmapfy-popup-modal-close popup-dismiss">×</div>

				<!-- content section  -->
				<div class="jltmapfy-popup-modal-footer">

					<!-- countdown  -->
					<div class="jltmapfy-popup-countdown" style="display: none;">
						<?php if (!empty($this->get_content('notice'))) { ?>
							<span data-counter="notice" style="color:#F4B740; font-size:14px; padding-bottom:20px; font-style:italic;">
								<?php echo esc_html__('Notice:', 'mapfy'); ?> <?php echo $this->get_content('notice'); ?>
							</span>
						<?php } ?>
						<span class="jltmapfy-popup-countdown-text"><?php echo esc_html__('Offer Ends In', 'mapfy'); ?></span>
						<div class="jltmapfy-popup-countdown-time">
							<div>
								<span data-counter="days">00</span>
								<span><?php echo esc_html__('Days', 'mapfy'); ?></span>
							</div>
							<span>:</span>
							<div>
								<span data-counter="hours">00</span>
								<span><?php echo esc_html__('Hours', 'mapfy'); ?></span>
							</div>
							<span>:</span>
							<div>
								<span data-counter="minutes">00</span>
								<span><?php echo esc_html__('Minutes', 'mapfy'); ?></span>
							</div>
							<span>:</span>
							<div>
								<span data-counter="seconds">00</span>
								<span><?php echo esc_html__('Seconds', 'mapfy'); ?></span>
							</div>
						</div>
					</div>

					<!-- button  -->
					<a class="jltmapfy-popup-button" target="_blank" href="<?php echo esc_url($this->get_content('button_url')); ?>"><?php echo esc_html($this->get_content('button_text')); ?></a>
				</div>
			</div>
		</div>

		<script>
			var $container = jQuery('#jltmapfy-popup'),
				plugin_data = <?php echo json_encode($this->get_sheet_promo_remote_data(), true); ?>,
				events = {}; //Events

			// Update Counter
			function updateCounter(seconds) {
				const $counter = $container.find(".jltmapfy-popup-countdown-time");
			...
			function initCounter(last_date) {
				$container.find(".jltmapfy-popup-countdown-time").show();
				...
				if (result) {
					...
					$container.find(".jltmapfy-popup-countdown").show(0);
				} else {
					trigger("countdownFinish", [plugin_data]);
					$container.find(".jltmapfy-popup-countdown").hide(0);
				}
